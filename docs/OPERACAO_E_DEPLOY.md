...
O Easypanel pode definir `PORT=80` automaticamente para a porta pública. A API ignora esse valor no processo supervisionado e usa internamente a porta `3001`.
...
- URLs como `/treinamentos` retornam 404: confirme que `/etc/nginx/conf.d/default.conf` recebeu o `nginx.conf` do repositório e que o deploy foi reconstruído sem cache.
